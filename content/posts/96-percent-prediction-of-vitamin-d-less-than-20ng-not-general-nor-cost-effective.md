---
title: "96% prediction of Vitamin D less than 20ng (not general nor cost-effective)"
slug: "96-percent-prediction-of-vitamin-d-less-than-20ng-not-general-nor-cost-effective"
aliases:
  - "/96+prediction+of+Vitamin+D+less+than+20ng+not+general+nor+cost-effective+-+May+2024"
  - "/15301"
tiki_page_id: 15301
date: 2024-05-27
categories:
  - "Predict Vitamin D"
---


{{< toc >}}

---

#### Vitamin D Deficiency Detection: A Novel Ensemble Approach with Interpretability Insights

IEEE Xplore: 23 May 2024 DOI: 10.1109/ICEEICT62016.2024.10534371 PDF cost VitaminDWiki $36

Published in: 2024 6th International Conference on Electrical Engineering and Information & Communication Technology (ICEEICT)

Date of Conference: 02-04 May 2024. Bangaldesh

Md.Fahim Ul Islam; Mehedi Hasan; Md.Tahmid Rahman; Amitabha Chakrabarty

Vitamin D deficiency is becoming a global public health concern, particularly in medical centers, with serious consequences for disease severity, mortality, and morbidity. Traditional diagnostic methodologies struggle with cost and are time-consuming. This drives a paradigm change toward the use of automated process for improved predicted accuracy and cost-effectiveness in diagnosing Vitamin D deficiency. In addressing the global challenge of Vitamin D deficiency, this study introduces an ensemble model that synergistically combines LightGBM and CatBoost algorithms, marking a significant leap in diagnostic methodologies. By leveraging the strengths of these advanced machine learning techniques, our approach achieves an impressive 96% accuracy on the Vitamin D Deficiency (VDD) Dataset, demonstrating substantial improvement over traditional diagnostic methods. The integration of SnAP (Shapley Additive explanations) for interpretability further enhances the utility of our model, providing clear insights into the impact of individual features on the severity predictions of Vitamin D deficiency. This revised abstract concisely encapsulates the research objectives, innovative methodology, and critical findings, underscoring the potential to revolutionize diagnostic efficiency and accuracy in the medical domain.

 **Variables for 3,000 college students** 

Weight (61-91 kgs), 

Height (1.48-1.73m), 

BMI (25.94-34.81 kg/m2), 

Waist Circumference (58-92 cm), 

Body Fat (21.60-41.20%),

Bone Mass (2.00-3.60), 

Exercise (yes/no),

Sunlight Exposure/Day (5.0-30 hours), >24 hours ??

Milk Consumption (0-500).

 **Used a combination of 5 machine learning techniques** 

Decision Tree

Support Vector Classifier (SVC), 

Naive Bayes (NB) model 

Multilayer Perceptron (MLP) 

Logistic Regression 

##### Does not mention which expensive Body Fat measurement was used

1. Hydrostatic Weighing (Underwater Weighing)

2. Air Displacement Plethysmography (ADP)

3. Bioelectrical Impedance Analysis (BIA)

4. Skinfold Measurements

5. Dual-Energy X-ray Absorptiometry (DEXA)

6. Magnetic Resonance Imaging (MRI) and Computed Tomography (CT) Scans

##### Does not mention which expensive Bone Mass measurement was used

Dual-Energy X-Ray Absorptiometry (DEXA or DXA)

Quantitative Computed Tomography (QCT)

Quantitative Ultrasound (QUS)

Peripheral Dual-Energy X-Ray Absorptiometry (pDXA)

Single-Energy X-Ray Absorptiometry (SEXA)

Radiographic Absorptiometry

Fracture Risk Assessment Tool (FRAX)

---

##### Ignores: latitude, season, skin color, seafood, supplementation, concealing clothing, high-risk job, health problems that consume Vitamin D, etc.

---

##### A $50 Vitamin D home test is less expensive AND provides exact level, not just < 20 ng

---

##### A $12 instant Vitamin D test provides Y/N 30 ng results


<pre style="background-color: #e0e0e0; white-space: pre-wrap;">
<code class="language-text">
Markdown:
--------
{{&lt; toc &gt;}}

---

#### Vitamin D Deficiency Detection: A Novel Ensemble Approach with Interpretability Insights

IEEE Xplore: 23 May 2024 DOI: 10.1109/ICEEICT62016.2024.10534371 PDF cost VitaminDWiki $36

Published in: 2024 6th International Conference on Electrical Engineering and Information &amp; Communication Technology (ICEEICT)

Date of Conference: 02-04 May 2024. Bangaldesh

Md.Fahim Ul Islam; Mehedi Hasan; Md.Tahmid Rahman; Amitabha Chakrabarty

Vitamin D deficiency is becoming a global public health concern, particularly in medical centers, with serious consequences for disease severity, mortality, and morbidity. Traditional diagnostic methodologies struggle with cost and are time-consuming. This drives a paradigm change toward the use of automated process for improved predicted accuracy and cost-effectiveness in diagnosing Vitamin D deficiency. In addressing the global challenge of Vitamin D deficiency, this study introduces an ensemble model that synergistically combines LightGBM and CatBoost algorithms, marking a significant leap in diagnostic methodologies. By leveraging the strengths of these advanced machine learning techniques, our approach achieves an impressive 96% accuracy on the Vitamin D Deficiency (VDD) Dataset, demonstrating substantial improvement over traditional diagnostic methods. The integration of SnAP (Shapley Additive explanations) for interpretability further enhances the utility of our model, providing clear insights into the impact of individual features on the severity predictions of Vitamin D deficiency. This revised abstract concisely encapsulates the research objectives, innovative methodology, and critical findings, underscoring the potential to revolutionize diagnostic efficiency and accuracy in the medical domain.

 **Variables for 3,000 college students** 

Weight (61-91 kgs), 

Height (1.48-1.73m), 

BMI (25.94-34.81 kg/m2), 

Waist Circumference (58-92 cm), 

Body Fat (21.60-41.20%),

Bone Mass (2.00-3.60), 

Exercise (yes/no),

Sunlight Exposure/Day (5.0-30 hours), &gt;24 hours ??

Milk Consumption (0-500).

 **Used a combination of 5 machine learning techniques** 

Decision Tree

Support Vector Classifier (SVC), 

Naive Bayes (NB) model 

Multilayer Perceptron (MLP) 

Logistic Regression 

##### Does not mention which expensive Body Fat measurement was used

1. Hydrostatic Weighing (Underwater Weighing)

2. Air Displacement Plethysmography (ADP)

3. Bioelectrical Impedance Analysis (BIA)

4. Skinfold Measurements

5. Dual-Energy X-ray Absorptiometry (DEXA)

6. Magnetic Resonance Imaging (MRI) and Computed Tomography (CT) Scans

##### Does not mention which expensive Bone Mass measurement was used

Dual-Energy X-Ray Absorptiometry (DEXA or DXA)

Quantitative Computed Tomography (QCT)

Quantitative Ultrasound (QUS)

Peripheral Dual-Energy X-Ray Absorptiometry (pDXA)

Single-Energy X-Ray Absorptiometry (SEXA)

Radiographic Absorptiometry

Fracture Risk Assessment Tool (FRAX)

---

##### Ignores: latitude, season, skin color, seafood, supplementation, concealing clothing, high-risk job, health problems that consume Vitamin D, etc.

---

##### A $50 Vitamin D home test is less expensive AND provides exact level, not just &lt; 20 ng

---

##### A $12 instant Vitamin D test provides Y/N 30 ng results


AST Structure:
-------------
├── DivNode
│   full_match: `{DIV(class=&quot;lefth4&quot;)}{maketoc Title=&quot;&quot;}{DIV}`
│   inner_content: `{maketoc Title=&quot;&quot;}`
│   attrs_dict:
│   │   raw_content: (class=&quot;lefth4&quot;)
│   │   class: lefth4
│   children:
│   ├── MakeTocNode
│   │   full_match: `{maketoc Title=&quot;&quot;}`
│   │   inner_content: ``
│   │   attrs_dict:
│   │   │   raw_content:  Title=&quot;&quot;
│   │   │   Title: ├── HorizontalRuleNode
│   full_match: `\n---\n`
│   inner_content: `---`├── HeadingNode
│   full_match: `!!!!Vitamin D Deficiency Detection: A Novel Ensemble Approach with Interpretability Insights`
│   inner_content: `Vitamin D Deficiency Detection: A Novel Ensemble Approach with Interpretability Insights`
│   level: `4`
│   children:
│   ├── TextNode
│   │   full_match: `Vitamin D Deficiency Detection: A Novel Ensemble Approach with Interpretability Insights`
│   │   inner_content: `Vitamin D Deficiency Detection: A Novel Ensemble Approach with Interpretability Insights`├── TextNode
│   full_match: `\nIEEE Xplore: 23 May 2024 DOI: 10.1109/ICEEICT62016.2024.10534371 PDF cost VitaminDWiki $36\nPublished in: 2024 6th International Conference on Electrical Engineering and Information &amp; Communication Technology (ICEEICT)\nDate of Conference: 02-04 May 2024. Bangaldesh\nMd.Fahim Ul Islam; Mehedi Hasan; Md.Tahmid Rahman; Amitabha Chakrabarty\n\nVitamin D deficiency is becoming a global public health concern, particularly in medical centers, with serious consequences for disease severity, mortality, and morbidity. Traditional diagnostic methodologies struggle with cost and are time-consuming. This drives a paradigm change toward the use of automated process for improved predicted accuracy and cost-effectiveness in diagnosing Vitamin D deficiency. In addressing the global challenge of Vitamin D deficiency, this study introduces an ensemble model that synergistically combines LightGBM and CatBoost algorithms, marking a significant leap in diagnostic methodologies. By leveraging the strengths of these advanced machine learning techniques, our approach achieves an impressive 96% accuracy on the Vitamin D Deficiency (VDD) Dataset, demonstrating substantial improvement over traditional diagnostic methods. The integration of SnAP (Shapley Additive explanations) for interpretability further enhances the utility of our model, providing clear insights into the impact of individual features on the severity predictions of Vitamin D deficiency. This revised abstract concisely encapsulates the research objectives, innovative methodology, and critical findings, underscoring the potential to revolutionize diagnostic efficiency and accuracy in the medical domain.\n\n`
│   inner_content: `\nIEEE Xplore: 23 May 2024 DOI: 10.1109/ICEEICT62016.2024.10534371 PDF cost VitaminDWiki $36\nPublished in: 2024 6th International Conference on Electrical Engineering and Information &amp; Communication Technology (ICEEICT)\nDate of Conference: 02-04 May 2024. Bangaldesh\nMd.Fahim Ul Islam; Mehedi Hasan; Md.Tahmid Rahman; Amitabha Chakrabarty\n\nVitamin D deficiency is becoming a global public health concern, particularly in medical centers, with serious consequences for disease severity, mortality, and morbidity. Traditional diagnostic methodologies struggle with cost and are time-consuming. This drives a paradigm change toward the use of automated process for improved predicted accuracy and cost-effectiveness in diagnosing Vitamin D deficiency. In addressing the global challenge of Vitamin D deficiency, this study introduces an ensemble model that synergistically combines LightGBM and CatBoost algorithms, marking a significant leap in diagnostic methodologies. By leveraging the strengths of these advanced machine learning techniques, our approach achieves an impressive 96% accuracy on the Vitamin D Deficiency (VDD) Dataset, demonstrating substantial improvement over traditional diagnostic methods. The integration of SnAP (Shapley Additive explanations) for interpretability further enhances the utility of our model, providing clear insights into the impact of individual features on the severity predictions of Vitamin D deficiency. This revised abstract concisely encapsulates the research objectives, innovative methodology, and critical findings, underscoring the potential to revolutionize diagnostic efficiency and accuracy in the medical domain.\n\n`├── BoldNode
│   full_match: `__Variables for 3,000 college students__`
│   inner_content: `Variables for 3,000 college students`
│   children:
│   ├── TextNode
│   │   full_match: `Variables for 3,000 college students`
│   │   inner_content: `Variables for 3,000 college students`├── TextNode
│   full_match: `\nWeight (61-91 kgs), \nHeight (1.48-1.73m), \nBMI (25.94-34.81 kg/m2), \nWaist Circumference (58-92 cm), \nBody Fat (21.60-41.20%),\nBone Mass (2.00-3.60), \nExercise (yes/no),\nSunlight Exposure/Day (5.0-30 hours), &gt;24 hours ??\nMilk Consumption (0-500).\n\n`
│   inner_content: `\nWeight (61-91 kgs), \nHeight (1.48-1.73m), \nBMI (25.94-34.81 kg/m2), \nWaist Circumference (58-92 cm), \nBody Fat (21.60-41.20%),\nBone Mass (2.00-3.60), \nExercise (yes/no),\nSunlight Exposure/Day (5.0-30 hours), &gt;24 hours ??\nMilk Consumption (0-500).\n\n`├── BoldNode
│   full_match: `__Used a combination of 5 machine learning techniques__`
│   inner_content: `Used a combination of 5 machine learning techniques`
│   children:
│   ├── TextNode
│   │   full_match: `Used a combination of 5 machine learning techniques`
│   │   inner_content: `Used a combination of 5 machine learning techniques`├── TextNode
│   full_match: `\nDecision Tree\nSupport Vector Classifier (SVC), \nNaive Bayes (NB) model \nMultilayer Perceptron (MLP) \nLogistic Regression \n`
│   inner_content: `\nDecision Tree\nSupport Vector Classifier (SVC), \nNaive Bayes (NB) model \nMultilayer Perceptron (MLP) \nLogistic Regression \n`├── HeadingNode
│   full_match: `\n!!!!!Does not mention which expensive Body Fat measurement was used`
│   inner_content: `Does not mention which expensive Body Fat measurement was used`
│   level: `5`
│   children:
│   ├── TextNode
│   │   full_match: `Does not mention which expensive Body Fat measurement was used`
│   │   inner_content: `Does not mention which expensive Body Fat measurement was used`├── TextNode
│   full_match: `\n1. Hydrostatic Weighing (Underwater Weighing)\n2. Air Displacement Plethysmography (ADP)\n3. Bioelectrical Impedance Analysis (BIA)\n4. Skinfold Measurements\n5. Dual-Energy X-ray Absorptiometry (DEXA)\n6. Magnetic Resonance Imaging (MRI) and Computed Tomography (CT) Scans\n`
│   inner_content: `\n1. Hydrostatic Weighing (Underwater Weighing)\n2. Air Displacement Plethysmography (ADP)\n3. Bioelectrical Impedance Analysis (BIA)\n4. Skinfold Measurements\n5. Dual-Energy X-ray Absorptiometry (DEXA)\n6. Magnetic Resonance Imaging (MRI) and Computed Tomography (CT) Scans\n`├── HeadingNode
│   full_match: `\n!!!!!Does not mention which expensive Bone Mass measurement was used`
│   inner_content: `Does not mention which expensive Bone Mass measurement was used`
│   level: `5`
│   children:
│   ├── TextNode
│   │   full_match: `Does not mention which expensive Bone Mass measurement was used`
│   │   inner_content: `Does not mention which expensive Bone Mass measurement was used`├── TextNode
│   full_match: `\nDual-Energy X-Ray Absorptiometry (DEXA or DXA)\nQuantitative Computed Tomography (QCT)\nQuantitative Ultrasound (QUS)\nPeripheral Dual-Energy X-Ray Absorptiometry (pDXA)\nSingle-Energy X-Ray Absorptiometry (SEXA)\nRadiographic Absorptiometry\nFracture Risk Assessment Tool (FRAX)`
│   inner_content: `\nDual-Energy X-Ray Absorptiometry (DEXA or DXA)\nQuantitative Computed Tomography (QCT)\nQuantitative Ultrasound (QUS)\nPeripheral Dual-Energy X-Ray Absorptiometry (pDXA)\nSingle-Energy X-Ray Absorptiometry (SEXA)\nRadiographic Absorptiometry\nFracture Risk Assessment Tool (FRAX)`├── HorizontalRuleNode
│   full_match: `\n---\n`
│   inner_content: `---`├── HeadingNode
│   full_match: `!!!!!Ignores: latitude, season, skin color, seafood, supplementation, concealing clothing, high-risk job, health problems that consume Vitamin D, etc.`
│   inner_content: `Ignores: latitude, season, skin color, seafood, supplementation, concealing clothing, high-risk job, health problems that consume Vitamin D, etc.`
│   level: `5`
│   children:
│   ├── TextNode
│   │   full_match: `Ignores: latitude, season, skin color, seafood, supplementation, concealing clothing, high-risk job, health problems that consume Vitamin D, etc.`
│   │   inner_content: `Ignores: latitude, season, skin color, seafood, supplementation, concealing clothing, high-risk job, health problems that consume Vitamin D, etc.`├── HorizontalRuleNode
│   full_match: `\n---\n`
│   inner_content: `---`├── HeadingNode
│   full_match: `!!!!!A $50 Vitamin D home test is less expensive AND provides exact level, not just &lt; 20 ng`
│   inner_content: `A $50 Vitamin D home test is less expensive AND provides exact level, not just &lt; 20 ng`
│   level: `5`
│   children:
│   ├── TextNode
│   │   full_match: `A $50 Vitamin D home test is less expensive AND provides exact level, not just &lt; 20 ng`
│   │   inner_content: `A $50 Vitamin D home test is less expensive AND provides exact level, not just &lt; 20 ng`├── HorizontalRuleNode
│   full_match: `\n---\n`
│   inner_content: `---`├── HeadingNode
│   full_match: `!!!!!A $12 instant Vitamin D test provides Y/N 30 ng results`
│   inner_content: `A $12 instant Vitamin D test provides Y/N 30 ng results`
│   level: `5`
│   children:
│   ├── TextNode
│   │   full_match: `A $12 instant Vitamin D test provides Y/N 30 ng results`
│   │   inner_content: `A $12 instant Vitamin D test provides Y/N 30 ng results`├── HorizontalRuleNode
│   full_match: `\n---\n`
│   inner_content: `---`├── HeadingNode
│   full_match: `!!!!VitaminDWiki – ((Predict Vitamin D)) category contains:`
│   inner_content: `VitaminDWiki – ((Predict Vitamin D)) category contains:`
│   level: `4`
│   children:
│   ├── TextNode
│   │   full_match: `VitaminDWiki – `
│   │   inner_content: `VitaminDWiki – `
│   ├── LocalLinkNode
│   │   full_match: `((Predict Vitamin D))`
│   │   inner_content: `Predict Vitamin D`
│   │   page: `Predict Vitamin D`
│   │   children:
│   │   ├── TextNode
│   │   │   full_match: `Predict Vitamin D`
│   │   │   inner_content: `Predict Vitamin D`
│   ├── TextNode
│   │   full_match: ` category contains:`
│   │   inner_content: ` category contains:`├── TextNode
│   full_match: `\n`
│   inner_content: `\n`├── IncludeNode
│   full_match: `{include page=&quot;Predict Vitamin D&quot; start=&quot;~tc~ start ~/tc~&quot; stop=&quot;~tc~ end ~/tc~&quot;}`
│   inner_content: ``
│   attrs_dict:
│   │   raw_content:  page=&quot;Predict Vitamin D&quot; start=&quot;~tc~ start ~/tc~&quot; stop=&quot;~tc~ end ~/tc~&quot;
│   │   page: Predict Vitamin D
│   │   start: ~tc~ start ~/tc~
│   │   stop: ~tc~ end ~/tc~
│   children:
│   ├── TextNode
│   │   full_match: ``
│   │   inner_content: ``├── TextNode
│   full_match: `\n\n`
│   inner_content: `\n\n`├── TikiCommentNode
│   full_match: `~tc~ (alias(predict IEEE)) ~/tc~`
│   inner_content: ` (alias(predict IEEE)) `
│   children:
│   ├── TextNode
│   │   full_match: `(alias(predict IEEE)) `
│   │   inner_content: `(alias(predict IEEE)) `├── TextNode
│   full_match: `\n`
│   inner_content: `\n`├── TikiCommentNode
│   full_match: `~tc~ (alias(96% prediction accuracy of Vitamin D levels using 5 AI techniques - May 2024)) ~/tc~`
│   inner_content: ` (alias(96% prediction accuracy of Vitamin D levels using 5 AI techniques - May 2024)) `
│   children:
│   ├── TextNode
│   │   full_match: `(alias(96% prediction accuracy of Vitamin D levels using 5 AI techniques - May 2024)) `
│   │   inner_content: `(alias(96% prediction accuracy of Vitamin D levels using 5 AI techniques - May 2024)) `├── TextNode
│   full_match: `\n`
│   inner_content: `\n`├── TikiCommentNode
│   full_match: `~tc~ (alias(96% prediction accuracy of Vitamin D less than 20ng (not general nor cost-effective) - May 2024)) ~/tc~`
│   inner_content: ` (alias(96% prediction accuracy of Vitamin D less than 20ng (not general nor cost-effective) - May 2024)) `
│   children:
│   ├── TextNode
│   │   full_match: `(alias(96% prediction accuracy of Vitamin D less than 20ng (not general nor cost-effective) - May 2024)) `
│   │   inner_content: `(alias(96% prediction accuracy of Vitamin D less than 20ng (not general nor cost-effective) - May 2024)) `

Original Tiki:
-------------
{DIV(class=&quot;lefth4&quot;)}{maketoc Title=&quot;&quot;}{DIV}
---
!!!!Vitamin D Deficiency Detection: A Novel Ensemble Approach with Interpretability Insights
IEEE Xplore: 23 May 2024 DOI: 10.1109/ICEEICT62016.2024.10534371 PDF cost VitaminDWiki $36
Published in: 2024 6th International Conference on Electrical Engineering and Information &amp; Communication Technology (ICEEICT)
Date of Conference: 02-04 May 2024. Bangaldesh
Md.Fahim Ul Islam; Mehedi Hasan; Md.Tahmid Rahman; Amitabha Chakrabarty

Vitamin D deficiency is becoming a global public health concern, particularly in medical centers, with serious consequences for disease severity, mortality, and morbidity. Traditional diagnostic methodologies struggle with cost and are time-consuming. This drives a paradigm change toward the use of automated process for improved predicted accuracy and cost-effectiveness in diagnosing Vitamin D deficiency. In addressing the global challenge of Vitamin D deficiency, this study introduces an ensemble model that synergistically combines LightGBM and CatBoost algorithms, marking a significant leap in diagnostic methodologies. By leveraging the strengths of these advanced machine learning techniques, our approach achieves an impressive 96% accuracy on the Vitamin D Deficiency (VDD) Dataset, demonstrating substantial improvement over traditional diagnostic methods. The integration of SnAP (Shapley Additive explanations) for interpretability further enhances the utility of our model, providing clear insights into the impact of individual features on the severity predictions of Vitamin D deficiency. This revised abstract concisely encapsulates the research objectives, innovative methodology, and critical findings, underscoring the potential to revolutionize diagnostic efficiency and accuracy in the medical domain.

__Variables for 3,000 college students__
Weight (61-91 kgs), 
Height (1.48-1.73m), 
BMI (25.94-34.81 kg/m2), 
Waist Circumference (58-92 cm), 
Body Fat (21.60-41.20%),
Bone Mass (2.00-3.60), 
Exercise (yes/no),
Sunlight Exposure/Day (5.0-30 hours), &gt;24 hours ??
Milk Consumption (0-500).

__Used a combination of 5 machine learning techniques__
Decision Tree
Support Vector Classifier (SVC), 
Naive Bayes (NB) model 
Multilayer Perceptron (MLP) 
Logistic Regression 

!!!!!Does not mention which expensive Body Fat measurement was used
1. Hydrostatic Weighing (Underwater Weighing)
2. Air Displacement Plethysmography (ADP)
3. Bioelectrical Impedance Analysis (BIA)
4. Skinfold Measurements
5. Dual-Energy X-ray Absorptiometry (DEXA)
6. Magnetic Resonance Imaging (MRI) and Computed Tomography (CT) Scans

!!!!!Does not mention which expensive Bone Mass measurement was used
Dual-Energy X-Ray Absorptiometry (DEXA or DXA)
Quantitative Computed Tomography (QCT)
Quantitative Ultrasound (QUS)
Peripheral Dual-Energy X-Ray Absorptiometry (pDXA)
Single-Energy X-Ray Absorptiometry (SEXA)
Radiographic Absorptiometry
Fracture Risk Assessment Tool (FRAX)
---
!!!!!Ignores: latitude, season, skin color, seafood, supplementation, concealing clothing, high-risk job, health problems that consume Vitamin D, etc.
---
!!!!!A $50 Vitamin D home test is less expensive AND provides exact level, not just &lt; 20 ng
---
!!!!!A $12 instant Vitamin D test provides Y/N 30 ng results
---
!!!!VitaminDWiki – ((Predict Vitamin D)) category contains:
{include page=&quot;Predict Vitamin D&quot; start=&quot;~tc~ start ~/tc~&quot; stop=&quot;~tc~ end ~/tc~&quot;}

~tc~ (alias(predict IEEE)) ~/tc~
~tc~ (alias(96% prediction accuracy of Vitamin D levels using 5 AI techniques - May 2024)) ~/tc~
~tc~ (alias(96% prediction accuracy of Vitamin D less than 20ng (not general nor cost-effective) - May 2024)) ~/tc~
</code>
</pre>
