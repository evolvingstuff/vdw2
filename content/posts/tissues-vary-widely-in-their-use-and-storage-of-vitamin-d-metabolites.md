---
title: "Tissues vary widely in their use and storage of Vitamin D metabolites"
slug: "tissues-vary-widely-in-their-use-and-storage-of-vitamin-d-metabolites"
aliases:
  - "/Tissues+vary+widely+in+their+use+and+storage+of+Vitamin+D+metabolites+–+Aug+2024"
  - "/15469"
tiki_page_id: 15469
date: 2024-08-12
---


<img src="https://d1bk1kqxc0sym.cloudfront.net/attachments/webp/vit-d-metabolites-vs-tissues.webp" alt="image" width="700">

{{< toc >}}

---

#### Vitamin D beyond blood: Tissue distribution of vitamin D metabolites after supplementation

Life Sciences  10 August 2024 https://doi.org/10.1016/j.lfs.2024.122942 PDF behind paywall

 **Highlights** 

* Vitamin D3 and its key metabolites mediate a variety of important autocrine and paracrine mechanisms.

* These non-endocrine functions highlight the importance of vitamin D3 tissue stores.

* Data on human tissue distribution of vitamin D3 metabolites is lacking, particularly in the context of pregnancy.

* Within the examined dosage ranges, both human and animal studies indicate that vitamin D3 and its metabolites are retained in a dose-dependent manner that is tissue-specific.

* The blood-tissue kinetics and dynamics of vitamin D metabolites after supplementation should be further investigated to inform supplementation strategies.

Vitamin D3's role in mineral homeostasis through its endocrine function, associated with the main circulating metabolite 25-hydroxyvitamin D3, is well characterized. However, the increasing recognition of vitamin D3's paracrine and autocrine functions—such as cell growth, immune function, and hormone regulation—necessitates examining vitamin D3 levels across different tissues post-supplementation. Hence, this review explores the biodistribution of vitamin D3 in blood and key tissues following oral supplementation in humans and animal models, highlighting the biologically active metabolite, 1,25-dihydroxyvitamin D3, and the primary clearance metabolite, 24,25-dihydroxyvitamin D3. While our findings indicate significant progress in understanding how circulating metabolite levels respond to supplementation, comprehensive insight into their tissue concentrations remains limited. The gap is particularly significant during pregnancy, a period of drastically increased vitamin D3 needs and metabolic alterations, where data remains sparse. Within the examined dosage ranges, both human and animal studies indicate that vitamin D3 and its metabolites are retained in tissues selectively.

Notably, vitamin D3 concentrations in tissues show greater variability in response to administered doses. In contrast, its metabolites maintain a more consistent concentration range, albeit different among tissues, reflecting their tighter regulatory mechanisms following supplementation. These observations suggest that serum 25-hydroxyvitamin D3 levels may not adequately reflect vitamin D3 and its metabolite concentrations in different tissues. Therefore, future research should aim to generate robust human data on the tissue distribution of vitamin D3 and its principal metabolites post-supplementation. Relating this data to clinically appropriate exposure metrics will enhance our understanding of vitamin D3's cellular effects and guide refinement of clinical trial methodologies.

 **Introduction** 

Vitamin D3 (VD), with its pleiotropic functional profile, plays an indispensable role in the regulation of bone mineral and immune homeostasis, and is deeply embedded in various physiological systems within the human body <sup>[1]</sup>. VD deficiency, a serum 25-hydroxyvitamin D (25VD) below 20 ng/ml as defined by the Institute of Medicine (IOM) based on skeletal health outcomes, is prevalent globally <sup>[2]</sup>, <sup>[3]</sup>, <sup>[4]</sup>, <sup>[5]</sup>. The prevalence of low VD status may be significantly higher when considering the Endocrine Society's definition of deficiency, which is a serum 25VD level of <30 ng/ml <sup>[6]</sup>. Despite the ubiquitous nature of VD deficiency, its health implications remain subject to debate. Numerous epidemiological studies have shown strong associations of VD deficiency with not only musculoskeletal health outcomes, but also a broad spectrum of adverse health outcomes, such as cancer, respiratory, neuro-developmental, auto-immune, cardiovascular and metabolic diseases <sup>[7]</sup>. On the other hand, randomized clinical trials (RCTs) examining the effect of oral VD supplementation on these health outcomes have generally been negative. This discrepancy has led some to question VD's role as a causal factor in non-skeletal health outcomes, implying association or even reverse-causality rather than a direct effect. However, the results of these RCT may have been inherently biased towards null findings. This bias can be attributed to the fact that the therapeutic-inspired research design and methodology of most trials failed to meet two critical requirements of nutritional intervention studies <sup>[8]</sup>. First, trials included large numbers of non-deficient subjects, diluting the potential to observe a true effect. Second, the one-size-fits-all (generic) approach to supplementation failed to ensure that all VD-deficient individuals achieved nutritional sufficiency <sup>[8]</sup>. When the intervention does not address a deficiency or fails to correct it adequately, evaluating the efficacy of the supplementation becomes impractical. This underscores a critical need for personalized supplementation regimens and reevaluation and novel insights into what constitutes VD sufficiency, extending beyond mere bone health <sup>[9]</sup>.

In recent years, novel approaches have been explored to redefine VD sufficiency. For example, the free hormone hypothesis stipulates that it is primarily unbound VD metabolites that enter cells to exert their effect <sup>[10]</sup>. Certain physiological and clinical conditions might alter the balance between total and free 25VD, suggesting that measuring the free fraction might improve assessments of VD status <sup>[11]</sup>. Moreover, 25VD might be increasingly considered a great biomarker of exposure, but not necessarily of effect. Therefore, there has been a growing clinical interest in metabolites bearing closer proximity to VD's physiological actions, such as 1,25-dihydroxyvitamin D (1,25VD), the main biologically active metabolite, and 24,25(OH)2D (24,25VD), the main clearance metabolite. Finally, VD metabolite ratios (VMR) might be useful for accurately evaluating supplementation efficacy <sup>[12]</sup>.

Refining supplementation efficacy necessitates not only a sense of the factors influencing supplementation response, such as body mass index (BMI) or interindividual variability in VD metabolism, but also a deep understanding of its distribution and subsequent target tissue concentrations required for an optimal physiological state <sup>[13]</sup>. As current guidelines are established based primarily on bone health, different health outcomes may benefit from varying thresholds <sup>[6]</sup>. More importantly, current research focusses almost exclusively on serum or plasma concentrations of VD and its metabolites, which does not fully capture the extent of tissue penetration post-supplementation. This oversight is significant because unlike drug trials that target specific serum concentrations to ensure immediate and systemic therapeutic efficacy, nutritional trials should aim for a more sustainable change in nutrient availability within the tissues and organs where its primary effects are exerted. Bridging this gap requires a profound understanding of how the body absorbs, distributes, and metabolizes the nutrient after circulatory exposure to establish a link between blood and tissue levels in both the general population and specific sub-populations, such as pregnant women. While extensive pharmacokinetic (PK) assessment is a pillar of the drug developmental process, it remains poorly characterized in the case of VD <sup>[14,15]</sup>. Therefore, the aim of this review is to elucidate the distribution of VD and its major metabolites in blood and tissue in response to oral VD supplementation. This knowledge is crucial in refining future trial designs and allows for more evidence-based evaluation of VD's efficacy in different body compartments.

 **Section snippets** 

Metabolism of vitamin D

Sunlight exposure (UVB) initiates the synthesis of VD in the skin and evolutionarily was the primary source of VD. However, most of the world's populations spend 95 % of their time indoors, and even those who venture outside may use sunscreen, which could decrease exposure to UVB radiation. While dietary intake can modestly support VD reserves, few foods are fortified to a great extent. In contrast, structured oral supplementation leads to significantly increased VD levels (D2 and D3) compared …

 **1,25-Dihydroxyvitamin D's endocrine, paracrine, and autocrine effects** 

The endocrine action of the active metabolite, 1,25VD, has been well-characterized in contributing to the maintenance of serum calcium and phosphate homeostasis through the regulation of intestinal and renal (re)absorption. Recent research has shifted focus towards 1,25VD's autocrine and/or paracrine activities within tissues as potentially important for non-skeletal related effects. While the endocrine effects of 1,25VD are linked to the serum levels of 25VD, its autocrine and paracrine …

 **Vitamin D's biodistribution in blood** 

The bioavailability of oral VD supplements exhibits significant inter-individual variability, with estimated rates between  **55 and 99 %**  <sup>[39]</sup>. Factors influencing VD absorption, and hence its bioavailability, include gastric and intestinal conditions (pH, gut enzymes and microbiome), other host-associated factors (baseline levels, age, morbidities and genetic variation <sup>[40]</sup>), and the supplement formulation (e.g., tablets, solutions, gels) <sup>[41,42]</sup>. Notably, VD absorption is not limited to passive …

 **Distribution of vitamin D and its metabolites across tissues following supplementation<span style="color:#00F;">(radioactive tracking)</span>** 

The 1972 study by Mawer et al. on the uptake of intravenously administered 14C-labelled VD in human tissues is foundational in elucidating the quantitative distribution and storage of VD following exogenous administration <sup>[65]</sup>. Analyzing data from seven patients, Mawer et al. discovered a consistent pattern of rapid serum clearance of total radioactivity, followed by its redistribution to various tissues.  **Notably, the liver, kidneys, lungs, and spleen accumulated a minimal portion, <2 %**  of the…

 **Vitamin D metabolite response during pregnancy<span style="color:#00F;">(1,25 VD can double)</span>** 

Pregnancy presents a unique physiological state during which VD metabolism is significantly altered with a surge in its concentrations, indicating an extensive physiological requirement for this hormone. During pregnancy, the conversion rate of VD to 25VD remains comparable to those in the non-pregnant state <sup>[120]</sup>. However, feedback regulation of 1,25VD becomes uncoupled, leading to a progressive increase in concentrations of 100–150 % upon conception and into the second trimester. This upward…

Discussion

Previous investigations have extensively characterized the changes to human serum 25VD after prolonged oral VD supplementation, identifying several individual-level modifiers of this response. Additionally, supplementation has been shown to increase serum 1,25VD in both pregnant and non-pregnant individuals, although significant increases in the latter are primarily observed during VD deficiency. Based on the summarized data from the reviewed studies (Table 1), it is evident that there's a…

Conclusions and future perspective

The current work provides insights into the distribution of VD and key metabolites throughout the body of various species. These findings have the potential to enhance research on the impact of VD supplementation, which may ultimately contribute to the development of more precise and effective supplementation guidelines as well as refined metrics for clinical studies. Future experimental research must rigorously investigate the serum and tissue levels of 1,25VD and 24,25VD in both humans and…


<pre style="background-color: #e0e0e0; white-space: pre-wrap;">
<code class="language-text">
Markdown:
--------
&lt;img src=&quot;https://d1bk1kqxc0sym.cloudfront.net/attachments/webp/vit-d-metabolites-vs-tissues.webp&quot; alt=&quot;image&quot; width=&quot;700&quot;&gt;

{{&lt; toc &gt;}}

---

#### Vitamin D beyond blood: Tissue distribution of vitamin D metabolites after supplementation

Life Sciences  10 August 2024 https://doi.org/10.1016/j.lfs.2024.122942 PDF behind paywall

 **Highlights** 

* Vitamin D3 and its key metabolites mediate a variety of important autocrine and paracrine mechanisms.

* These non-endocrine functions highlight the importance of vitamin D3 tissue stores.

* Data on human tissue distribution of vitamin D3 metabolites is lacking, particularly in the context of pregnancy.

* Within the examined dosage ranges, both human and animal studies indicate that vitamin D3 and its metabolites are retained in a dose-dependent manner that is tissue-specific.

* The blood-tissue kinetics and dynamics of vitamin D metabolites after supplementation should be further investigated to inform supplementation strategies.

Vitamin D3&#39;s role in mineral homeostasis through its endocrine function, associated with the main circulating metabolite 25-hydroxyvitamin D3, is well characterized. However, the increasing recognition of vitamin D3&#39;s paracrine and autocrine functions—such as cell growth, immune function, and hormone regulation—necessitates examining vitamin D3 levels across different tissues post-supplementation. Hence, this review explores the biodistribution of vitamin D3 in blood and key tissues following oral supplementation in humans and animal models, highlighting the biologically active metabolite, 1,25-dihydroxyvitamin D3, and the primary clearance metabolite, 24,25-dihydroxyvitamin D3. While our findings indicate significant progress in understanding how circulating metabolite levels respond to supplementation, comprehensive insight into their tissue concentrations remains limited. The gap is particularly significant during pregnancy, a period of drastically increased vitamin D3 needs and metabolic alterations, where data remains sparse. Within the examined dosage ranges, both human and animal studies indicate that vitamin D3 and its metabolites are retained in tissues selectively.

Notably, vitamin D3 concentrations in tissues show greater variability in response to administered doses. In contrast, its metabolites maintain a more consistent concentration range, albeit different among tissues, reflecting their tighter regulatory mechanisms following supplementation. These observations suggest that serum 25-hydroxyvitamin D3 levels may not adequately reflect vitamin D3 and its metabolite concentrations in different tissues. Therefore, future research should aim to generate robust human data on the tissue distribution of vitamin D3 and its principal metabolites post-supplementation. Relating this data to clinically appropriate exposure metrics will enhance our understanding of vitamin D3&#39;s cellular effects and guide refinement of clinical trial methodologies.

 **Introduction** 

Vitamin D3 (VD), with its pleiotropic functional profile, plays an indispensable role in the regulation of bone mineral and immune homeostasis, and is deeply embedded in various physiological systems within the human body &lt;sup&gt;[1]&lt;/sup&gt;. VD deficiency, a serum 25-hydroxyvitamin D (25VD) below 20 ng/ml as defined by the Institute of Medicine (IOM) based on skeletal health outcomes, is prevalent globally &lt;sup&gt;[2]&lt;/sup&gt;, &lt;sup&gt;[3]&lt;/sup&gt;, &lt;sup&gt;[4]&lt;/sup&gt;, &lt;sup&gt;[5]&lt;/sup&gt;. The prevalence of low VD status may be significantly higher when considering the Endocrine Society&#39;s definition of deficiency, which is a serum 25VD level of &lt;30 ng/ml &lt;sup&gt;[6]&lt;/sup&gt;. Despite the ubiquitous nature of VD deficiency, its health implications remain subject to debate. Numerous epidemiological studies have shown strong associations of VD deficiency with not only musculoskeletal health outcomes, but also a broad spectrum of adverse health outcomes, such as cancer, respiratory, neuro-developmental, auto-immune, cardiovascular and metabolic diseases &lt;sup&gt;[7]&lt;/sup&gt;. On the other hand, randomized clinical trials (RCTs) examining the effect of oral VD supplementation on these health outcomes have generally been negative. This discrepancy has led some to question VD&#39;s role as a causal factor in non-skeletal health outcomes, implying association or even reverse-causality rather than a direct effect. However, the results of these RCT may have been inherently biased towards null findings. This bias can be attributed to the fact that the therapeutic-inspired research design and methodology of most trials failed to meet two critical requirements of nutritional intervention studies &lt;sup&gt;[8]&lt;/sup&gt;. First, trials included large numbers of non-deficient subjects, diluting the potential to observe a true effect. Second, the one-size-fits-all (generic) approach to supplementation failed to ensure that all VD-deficient individuals achieved nutritional sufficiency &lt;sup&gt;[8]&lt;/sup&gt;. When the intervention does not address a deficiency or fails to correct it adequately, evaluating the efficacy of the supplementation becomes impractical. This underscores a critical need for personalized supplementation regimens and reevaluation and novel insights into what constitutes VD sufficiency, extending beyond mere bone health &lt;sup&gt;[9]&lt;/sup&gt;.

In recent years, novel approaches have been explored to redefine VD sufficiency. For example, the free hormone hypothesis stipulates that it is primarily unbound VD metabolites that enter cells to exert their effect &lt;sup&gt;[10]&lt;/sup&gt;. Certain physiological and clinical conditions might alter the balance between total and free 25VD, suggesting that measuring the free fraction might improve assessments of VD status &lt;sup&gt;[11]&lt;/sup&gt;. Moreover, 25VD might be increasingly considered a great biomarker of exposure, but not necessarily of effect. Therefore, there has been a growing clinical interest in metabolites bearing closer proximity to VD&#39;s physiological actions, such as 1,25-dihydroxyvitamin D (1,25VD), the main biologically active metabolite, and 24,25(OH)2D (24,25VD), the main clearance metabolite. Finally, VD metabolite ratios (VMR) might be useful for accurately evaluating supplementation efficacy &lt;sup&gt;[12]&lt;/sup&gt;.

Refining supplementation efficacy necessitates not only a sense of the factors influencing supplementation response, such as body mass index (BMI) or interindividual variability in VD metabolism, but also a deep understanding of its distribution and subsequent target tissue concentrations required for an optimal physiological state &lt;sup&gt;[13]&lt;/sup&gt;. As current guidelines are established based primarily on bone health, different health outcomes may benefit from varying thresholds &lt;sup&gt;[6]&lt;/sup&gt;. More importantly, current research focusses almost exclusively on serum or plasma concentrations of VD and its metabolites, which does not fully capture the extent of tissue penetration post-supplementation. This oversight is significant because unlike drug trials that target specific serum concentrations to ensure immediate and systemic therapeutic efficacy, nutritional trials should aim for a more sustainable change in nutrient availability within the tissues and organs where its primary effects are exerted. Bridging this gap requires a profound understanding of how the body absorbs, distributes, and metabolizes the nutrient after circulatory exposure to establish a link between blood and tissue levels in both the general population and specific sub-populations, such as pregnant women. While extensive pharmacokinetic (PK) assessment is a pillar of the drug developmental process, it remains poorly characterized in the case of VD &lt;sup&gt;[14,15]&lt;/sup&gt;. Therefore, the aim of this review is to elucidate the distribution of VD and its major metabolites in blood and tissue in response to oral VD supplementation. This knowledge is crucial in refining future trial designs and allows for more evidence-based evaluation of VD&#39;s efficacy in different body compartments.

 **Section snippets** 

Metabolism of vitamin D

Sunlight exposure (UVB) initiates the synthesis of VD in the skin and evolutionarily was the primary source of VD. However, most of the world&#39;s populations spend 95 % of their time indoors, and even those who venture outside may use sunscreen, which could decrease exposure to UVB radiation. While dietary intake can modestly support VD reserves, few foods are fortified to a great extent. In contrast, structured oral supplementation leads to significantly increased VD levels (D2 and D3) compared …

 **1,25-Dihydroxyvitamin D&#39;s endocrine, paracrine, and autocrine effects** 

The endocrine action of the active metabolite, 1,25VD, has been well-characterized in contributing to the maintenance of serum calcium and phosphate homeostasis through the regulation of intestinal and renal (re)absorption. Recent research has shifted focus towards 1,25VD&#39;s autocrine and/or paracrine activities within tissues as potentially important for non-skeletal related effects. While the endocrine effects of 1,25VD are linked to the serum levels of 25VD, its autocrine and paracrine …

 **Vitamin D&#39;s biodistribution in blood** 

The bioavailability of oral VD supplements exhibits significant inter-individual variability, with estimated rates between  **55 and 99 %**  &lt;sup&gt;[39]&lt;/sup&gt;. Factors influencing VD absorption, and hence its bioavailability, include gastric and intestinal conditions (pH, gut enzymes and microbiome), other host-associated factors (baseline levels, age, morbidities and genetic variation &lt;sup&gt;[40]&lt;/sup&gt;), and the supplement formulation (e.g., tablets, solutions, gels) &lt;sup&gt;[41,42]&lt;/sup&gt;. Notably, VD absorption is not limited to passive …

 **Distribution of vitamin D and its metabolites across tissues following supplementation&lt;span style=&quot;color:#00F;&quot;&gt;(radioactive tracking)&lt;/span&gt;** 

The 1972 study by Mawer et al. on the uptake of intravenously administered 14C-labelled VD in human tissues is foundational in elucidating the quantitative distribution and storage of VD following exogenous administration &lt;sup&gt;[65]&lt;/sup&gt;. Analyzing data from seven patients, Mawer et al. discovered a consistent pattern of rapid serum clearance of total radioactivity, followed by its redistribution to various tissues.  **Notably, the liver, kidneys, lungs, and spleen accumulated a minimal portion, &lt;2 %**  of the…

 **Vitamin D metabolite response during pregnancy&lt;span style=&quot;color:#00F;&quot;&gt;(1,25 VD can double)&lt;/span&gt;** 

Pregnancy presents a unique physiological state during which VD metabolism is significantly altered with a surge in its concentrations, indicating an extensive physiological requirement for this hormone. During pregnancy, the conversion rate of VD to 25VD remains comparable to those in the non-pregnant state &lt;sup&gt;[120]&lt;/sup&gt;. However, feedback regulation of 1,25VD becomes uncoupled, leading to a progressive increase in concentrations of 100–150 % upon conception and into the second trimester. This upward…

Discussion

Previous investigations have extensively characterized the changes to human serum 25VD after prolonged oral VD supplementation, identifying several individual-level modifiers of this response. Additionally, supplementation has been shown to increase serum 1,25VD in both pregnant and non-pregnant individuals, although significant increases in the latter are primarily observed during VD deficiency. Based on the summarized data from the reviewed studies (Table 1), it is evident that there&#39;s a…

Conclusions and future perspective

The current work provides insights into the distribution of VD and key metabolites throughout the body of various species. These findings have the potential to enhance research on the impact of VD supplementation, which may ultimately contribute to the development of more precise and effective supplementation guidelines as well as refined metrics for clinical studies. Future experimental research must rigorously investigate the serum and tissue levels of 1,25VD and 24,25VD in both humans and…


AST Structure:
-------------
├── ImgNode
│   full_match: `{img type=&quot;attId&quot; attId=&quot;21562&quot; width=&quot;700&quot;}`
│   inner_content: ``
│   attrs_dict:
│   │   raw_content:  type=&quot;attId&quot; attId=&quot;21562&quot; width=&quot;700&quot;
│   │   type: attId
│   │   attId: 21562
│   │   width: 700├── TextNode
│   full_match: `\n\n`
│   inner_content: `\n\n`├── DivNode
│   full_match: `{DIV(class=&quot;lefth4&quot;)}{maketoc Title=&quot;&quot;}{DIV}`
│   inner_content: `{maketoc Title=&quot;&quot;}`
│   attrs_dict:
│   │   raw_content: (class=&quot;lefth4&quot;)
│   │   class: lefth4
│   children:
│   ├── MakeTocNode
│   │   full_match: `{maketoc Title=&quot;&quot;}`
│   │   inner_content: ``
│   │   attrs_dict:
│   │   │   raw_content:  Title=&quot;&quot;
│   │   │   Title: ├── HorizontalRuleNode
│   full_match: `\n---\n`
│   inner_content: `---`├── HeadingNode
│   full_match: `!!!!Vitamin D beyond blood: Tissue distribution of vitamin D metabolites after supplementation`
│   inner_content: `Vitamin D beyond blood: Tissue distribution of vitamin D metabolites after supplementation`
│   level: `4`
│   children:
│   ├── TextNode
│   │   full_match: `Vitamin D beyond blood: Tissue distribution of vitamin D metabolites after supplementation`
│   │   inner_content: `Vitamin D beyond blood: Tissue distribution of vitamin D metabolites after supplementation`├── TextNode
│   full_match: `\nLife Sciences  10 August 2024 https://doi.org/10.1016/j.lfs.2024.122942 PDF behind paywall\n\n`
│   inner_content: `\nLife Sciences  10 August 2024 https://doi.org/10.1016/j.lfs.2024.122942 PDF behind paywall\n\n`├── BoldNode
│   full_match: `__Highlights__`
│   inner_content: `Highlights`
│   children:
│   ├── TextNode
│   │   full_match: `Highlights`
│   │   inner_content: `Highlights`├── TextNode
│   full_match: `\n`
│   inner_content: `\n`├── ListItemNode
│   full_match: `*Vitamin D3 and its key metabolites mediate a variety of important autocrine and paracrine mechanisms.`
│   inner_content: `Vitamin D3 and its key metabolites mediate a variety of important autocrine and paracrine mechanisms.`
│   depth: `1`
│   children:
│   ├── TextNode
│   │   full_match: `Vitamin D3 and its key metabolites mediate a variety of important autocrine and paracrine mechanisms.`
│   │   inner_content: `Vitamin D3 and its key metabolites mediate a variety of important autocrine and paracrine mechanisms.`├── TextNode
│   full_match: `\n`
│   inner_content: `\n`├── ListItemNode
│   full_match: `*These non-endocrine functions highlight the importance of vitamin D3 tissue stores.`
│   inner_content: `These non-endocrine functions highlight the importance of vitamin D3 tissue stores.`
│   depth: `1`
│   children:
│   ├── TextNode
│   │   full_match: `These non-endocrine functions highlight the importance of vitamin D3 tissue stores.`
│   │   inner_content: `These non-endocrine functions highlight the importance of vitamin D3 tissue stores.`├── TextNode
│   full_match: `\n`
│   inner_content: `\n`├── ListItemNode
│   full_match: `*Data on human tissue distribution of vitamin D3 metabolites is lacking, particularly in the context of pregnancy.`
│   inner_content: `Data on human tissue distribution of vitamin D3 metabolites is lacking, particularly in the context of pregnancy.`
│   depth: `1`
│   children:
│   ├── TextNode
│   │   full_match: `Data on human tissue distribution of vitamin D3 metabolites is lacking, particularly in the context of pregnancy.`
│   │   inner_content: `Data on human tissue distribution of vitamin D3 metabolites is lacking, particularly in the context of pregnancy.`├── TextNode
│   full_match: `\n`
│   inner_content: `\n`├── ListItemNode
│   full_match: `*Within the examined dosage ranges, both human and animal studies indicate that vitamin D3 and its metabolites are retained in a dose-dependent manner that is tissue-specific.`
│   inner_content: `Within the examined dosage ranges, both human and animal studies indicate that vitamin D3 and its metabolites are retained in a dose-dependent manner that is tissue-specific.`
│   depth: `1`
│   children:
│   ├── TextNode
│   │   full_match: `Within the examined dosage ranges, both human and animal studies indicate that vitamin D3 and its metabolites are retained in a dose-dependent manner that is tissue-specific.`
│   │   inner_content: `Within the examined dosage ranges, both human and animal studies indicate that vitamin D3 and its metabolites are retained in a dose-dependent manner that is tissue-specific.`├── TextNode
│   full_match: `\n`
│   inner_content: `\n`├── ListItemNode
│   full_match: `*The blood-tissue kinetics and dynamics of vitamin D metabolites after supplementation should be further investigated to inform supplementation strategies.`
│   inner_content: `The blood-tissue kinetics and dynamics of vitamin D metabolites after supplementation should be further investigated to inform supplementation strategies.`
│   depth: `1`
│   children:
│   ├── TextNode
│   │   full_match: `The blood-tissue kinetics and dynamics of vitamin D metabolites after supplementation should be further investigated to inform supplementation strategies.`
│   │   inner_content: `The blood-tissue kinetics and dynamics of vitamin D metabolites after supplementation should be further investigated to inform supplementation strategies.`├── TextNode
│   full_match: `\nVitamin D3&#39;s role in mineral homeostasis through its endocrine function, associated with the main circulating metabolite 25-hydroxyvitamin D3, is well characterized. However, the increasing recognition of vitamin D3&#39;s paracrine and autocrine functions—such as cell growth, immune function, and hormone regulation—necessitates examining vitamin D3 levels across different tissues post-supplementation. Hence, this review explores the biodistribution of vitamin D3 in blood and key tissues following oral supplementation in humans and animal models, highlighting the biologically active metabolite, 1,25-dihydroxyvitamin D3, and the primary clearance metabolite, 24,25-dihydroxyvitamin D3. While our findings indicate significant progress in understanding how circulating metabolite levels respond to supplementation, comprehensive insight into their tissue concentrations remains limited. The gap is particularly significant during pregnancy, a period of drastically increased vitamin D3 needs and metabolic alterations, where data remains sparse. Within the examined dosage ranges, both human and animal studies indicate that vitamin D3 and its metabolites are retained in tissues selectively.\nNotably, vitamin D3 concentrations in tissues show greater variability in response to administered doses. In contrast, its metabolites maintain a more consistent concentration range, albeit different among tissues, reflecting their tighter regulatory mechanisms following supplementation. These observations suggest that serum 25-hydroxyvitamin D3 levels may not adequately reflect vitamin D3 and its metabolite concentrations in different tissues. Therefore, future research should aim to generate robust human data on the tissue distribution of vitamin D3 and its principal metabolites post-supplementation. Relating this data to clinically appropriate exposure metrics will enhance our understanding of vitamin D3&#39;s cellular effects and guide refinement of clinical trial methodologies.\n\n`
│   inner_content: `\nVitamin D3&#39;s role in mineral homeostasis through its endocrine function, associated with the main circulating metabolite 25-hydroxyvitamin D3, is well characterized. However, the increasing recognition of vitamin D3&#39;s paracrine and autocrine functions—such as cell growth, immune function, and hormone regulation—necessitates examining vitamin D3 levels across different tissues post-supplementation. Hence, this review explores the biodistribution of vitamin D3 in blood and key tissues following oral supplementation in humans and animal models, highlighting the biologically active metabolite, 1,25-dihydroxyvitamin D3, and the primary clearance metabolite, 24,25-dihydroxyvitamin D3. While our findings indicate significant progress in understanding how circulating metabolite levels respond to supplementation, comprehensive insight into their tissue concentrations remains limited. The gap is particularly significant during pregnancy, a period of drastically increased vitamin D3 needs and metabolic alterations, where data remains sparse. Within the examined dosage ranges, both human and animal studies indicate that vitamin D3 and its metabolites are retained in tissues selectively.\nNotably, vitamin D3 concentrations in tissues show greater variability in response to administered doses. In contrast, its metabolites maintain a more consistent concentration range, albeit different among tissues, reflecting their tighter regulatory mechanisms following supplementation. These observations suggest that serum 25-hydroxyvitamin D3 levels may not adequately reflect vitamin D3 and its metabolite concentrations in different tissues. Therefore, future research should aim to generate robust human data on the tissue distribution of vitamin D3 and its principal metabolites post-supplementation. Relating this data to clinically appropriate exposure metrics will enhance our understanding of vitamin D3&#39;s cellular effects and guide refinement of clinical trial methodologies.\n\n`├── BoldNode
│   full_match: `__Introduction__`
│   inner_content: `Introduction`
│   children:
│   ├── TextNode
│   │   full_match: `Introduction`
│   │   inner_content: `Introduction`├── TextNode
│   full_match: `\nVitamin D3 (VD), with its pleiotropic functional profile, plays an indispensable role in the regulation of bone mineral and immune homeostasis, and is deeply embedded in various physiological systems within the human body `
│   inner_content: `\nVitamin D3 (VD), with its pleiotropic functional profile, plays an indispensable role in the regulation of bone mineral and immune homeostasis, and is deeply embedded in various physiological systems within the human body `├── LinkNode
│   full_match: `[1]`
│   inner_content: `1`
│   url: `1`
│   children:
│   ├── TextNode
│   │   full_match: `1`
│   │   inner_content: `1`├── TextNode
│   full_match: `. VD deficiency, a serum 25-hydroxyvitamin D (25VD) below 20 ng/ml as defined by the Institute of Medicine (IOM) based on skeletal health outcomes, is prevalent globally `
│   inner_content: `. VD deficiency, a serum 25-hydroxyvitamin D (25VD) below 20 ng/ml as defined by the Institute of Medicine (IOM) based on skeletal health outcomes, is prevalent globally `├── LinkNode
│   full_match: `[2]`
│   inner_content: `2`
│   url: `2`
│   children:
│   ├── TextNode
│   │   full_match: `2`
│   │   inner_content: `2`├── TextNode
│   full_match: `, `
│   inner_content: `, `├── LinkNode
│   full_match: `[3]`
│   inner_content: `3`
│   url: `3`
│   children:
│   ├── TextNode
│   │   full_match: `3`
│   │   inner_content: `3`├── TextNode
│   full_match: `, `
│   inner_content: `, `├── LinkNode
│   full_match: `[4]`
│   inner_content: `4`
│   url: `4`
│   children:
│   ├── TextNode
│   │   full_match: `4`
│   │   inner_content: `4`├── TextNode
│   full_match: `, `
│   inner_content: `, `├── LinkNode
│   full_match: `[5]`
│   inner_content: `5`
│   url: `5`
│   children:
│   ├── TextNode
│   │   full_match: `5`
│   │   inner_content: `5`├── TextNode
│   full_match: `. The prevalence of low VD status may be significantly higher when considering the Endocrine Society&#39;s definition of deficiency, which is a serum 25VD level of &lt;30 ng/ml `
│   inner_content: `. The prevalence of low VD status may be significantly higher when considering the Endocrine Society&#39;s definition of deficiency, which is a serum 25VD level of &lt;30 ng/ml `├── LinkNode
│   full_match: `[6]`
│   inner_content: `6`
│   url: `6`
│   children:
│   ├── TextNode
│   │   full_match: `6`
│   │   inner_content: `6`├── TextNode
│   full_match: `. Despite the ubiquitous nature of VD deficiency, its health implications remain subject to debate. Numerous epidemiological studies have shown strong associations of VD deficiency with not only musculoskeletal health outcomes, but also a broad spectrum of adverse health outcomes, such as cancer, respiratory, neuro-developmental, auto-immune, cardiovascular and metabolic diseases `
│   inner_content: `. Despite the ubiquitous nature of VD deficiency, its health implications remain subject to debate. Numerous epidemiological studies have shown strong associations of VD deficiency with not only musculoskeletal health outcomes, but also a broad spectrum of adverse health outcomes, such as cancer, respiratory, neuro-developmental, auto-immune, cardiovascular and metabolic diseases `├── LinkNode
│   full_match: `[7]`
│   inner_content: `7`
│   url: `7`
│   children:
│   ├── TextNode
│   │   full_match: `7`
│   │   inner_content: `7`├── TextNode
│   full_match: `. On the other hand, randomized clinical trials (RCTs) examining the effect of oral VD supplementation on these health outcomes have generally been negative. This discrepancy has led some to question VD&#39;s role as a causal factor in non-skeletal health outcomes, implying association or even reverse-causality rather than a direct effect. However, the results of these RCT may have been inherently biased towards null findings. This bias can be attributed to the fact that the therapeutic-inspired research design and methodology of most trials failed to meet two critical requirements of nutritional intervention studies `
│   inner_content: `. On the other hand, randomized clinical trials (RCTs) examining the effect of oral VD supplementation on these health outcomes have generally been negative. This discrepancy has led some to question VD&#39;s role as a causal factor in non-skeletal health outcomes, implying association or even reverse-causality rather than a direct effect. However, the results of these RCT may have been inherently biased towards null findings. This bias can be attributed to the fact that the therapeutic-inspired research design and methodology of most trials failed to meet two critical requirements of nutritional intervention studies `├── LinkNode
│   full_match: `[8]`
│   inner_content: `8`
│   url: `8`
│   children:
│   ├── TextNode
│   │   full_match: `8`
│   │   inner_content: `8`├── TextNode
│   full_match: `. First, trials included large numbers of non-deficient subjects, diluting the potential to observe a true effect. Second, the one-size-fits-all (generic) approach to supplementation failed to ensure that all VD-deficient individuals achieved nutritional sufficiency `
│   inner_content: `. First, trials included large numbers of non-deficient subjects, diluting the potential to observe a true effect. Second, the one-size-fits-all (generic) approach to supplementation failed to ensure that all VD-deficient individuals achieved nutritional sufficiency `├── LinkNode
│   full_match: `[8]`
│   inner_content: `8`
│   url: `8`
│   children:
│   ├── TextNode
│   │   full_match: `8`
│   │   inner_content: `8`├── TextNode
│   full_match: `. When the intervention does not address a deficiency or fails to correct it adequately, evaluating the efficacy of the supplementation becomes impractical. This underscores a critical need for personalized supplementation regimens and reevaluation and novel insights into what constitutes VD sufficiency, extending beyond mere bone health `
│   inner_content: `. When the intervention does not address a deficiency or fails to correct it adequately, evaluating the efficacy of the supplementation becomes impractical. This underscores a critical need for personalized supplementation regimens and reevaluation and novel insights into what constitutes VD sufficiency, extending beyond mere bone health `├── LinkNode
│   full_match: `[9]`
│   inner_content: `9`
│   url: `9`
│   children:
│   ├── TextNode
│   │   full_match: `9`
│   │   inner_content: `9`├── TextNode
│   full_match: `.\n\nIn recent years, novel approaches have been explored to redefine VD sufficiency. For example, the free hormone hypothesis stipulates that it is primarily unbound VD metabolites that enter cells to exert their effect `
│   inner_content: `.\n\nIn recent years, novel approaches have been explored to redefine VD sufficiency. For example, the free hormone hypothesis stipulates that it is primarily unbound VD metabolites that enter cells to exert their effect `├── LinkNode
│   full_match: `[10]`
│   inner_content: `10`
│   url: `10`
│   children:
│   ├── TextNode
│   │   full_match: `10`
│   │   inner_content: `10`├── TextNode
│   full_match: `. Certain physiological and clinical conditions might alter the balance between total and free 25VD, suggesting that measuring the free fraction might improve assessments of VD status `
│   inner_content: `. Certain physiological and clinical conditions might alter the balance between total and free 25VD, suggesting that measuring the free fraction might improve assessments of VD status `├── LinkNode
│   full_match: `[11]`
│   inner_content: `11`
│   url: `11`
│   children:
│   ├── TextNode
│   │   full_match: `11`
│   │   inner_content: `11`├── TextNode
│   full_match: `. Moreover, 25VD might be increasingly considered a great biomarker of exposure, but not necessarily of effect. Therefore, there has been a growing clinical interest in metabolites bearing closer proximity to VD&#39;s physiological actions, such as 1,25-dihydroxyvitamin D (1,25VD), the main biologically active metabolite, and 24,25(OH)2D (24,25VD), the main clearance metabolite. Finally, VD metabolite ratios (VMR) might be useful for accurately evaluating supplementation efficacy `
│   inner_content: `. Moreover, 25VD might be increasingly considered a great biomarker of exposure, but not necessarily of effect. Therefore, there has been a growing clinical interest in metabolites bearing closer proximity to VD&#39;s physiological actions, such as 1,25-dihydroxyvitamin D (1,25VD), the main biologically active metabolite, and 24,25(OH)2D (24,25VD), the main clearance metabolite. Finally, VD metabolite ratios (VMR) might be useful for accurately evaluating supplementation efficacy `├── LinkNode
│   full_match: `[12]`
│   inner_content: `12`
│   url: `12`
│   children:
│   ├── TextNode
│   │   full_match: `12`
│   │   inner_content: `12`├── TextNode
│   full_match: `.\n\nRefining supplementation efficacy necessitates not only a sense of the factors influencing supplementation response, such as body mass index (BMI) or interindividual variability in VD metabolism, but also a deep understanding of its distribution and subsequent target tissue concentrations required for an optimal physiological state `
│   inner_content: `.\n\nRefining supplementation efficacy necessitates not only a sense of the factors influencing supplementation response, such as body mass index (BMI) or interindividual variability in VD metabolism, but also a deep understanding of its distribution and subsequent target tissue concentrations required for an optimal physiological state `├── LinkNode
│   full_match: `[13]`
│   inner_content: `13`
│   url: `13`
│   children:
│   ├── TextNode
│   │   full_match: `13`
│   │   inner_content: `13`├── TextNode
│   full_match: `. As current guidelines are established based primarily on bone health, different health outcomes may benefit from varying thresholds `
│   inner_content: `. As current guidelines are established based primarily on bone health, different health outcomes may benefit from varying thresholds `├── LinkNode
│   full_match: `[6]`
│   inner_content: `6`
│   url: `6`
│   children:
│   ├── TextNode
│   │   full_match: `6`
│   │   inner_content: `6`├── TextNode
│   full_match: `. More importantly, current research focusses almost exclusively on serum or plasma concentrations of VD and its metabolites, which does not fully capture the extent of tissue penetration post-supplementation. This oversight is significant because unlike drug trials that target specific serum concentrations to ensure immediate and systemic therapeutic efficacy, nutritional trials should aim for a more sustainable change in nutrient availability within the tissues and organs where its primary effects are exerted. Bridging this gap requires a profound understanding of how the body absorbs, distributes, and metabolizes the nutrient after circulatory exposure to establish a link between blood and tissue levels in both the general population and specific sub-populations, such as pregnant women. While extensive pharmacokinetic (PK) assessment is a pillar of the drug developmental process, it remains poorly characterized in the case of VD `
│   inner_content: `. More importantly, current research focusses almost exclusively on serum or plasma concentrations of VD and its metabolites, which does not fully capture the extent of tissue penetration post-supplementation. This oversight is significant because unlike drug trials that target specific serum concentrations to ensure immediate and systemic therapeutic efficacy, nutritional trials should aim for a more sustainable change in nutrient availability within the tissues and organs where its primary effects are exerted. Bridging this gap requires a profound understanding of how the body absorbs, distributes, and metabolizes the nutrient after circulatory exposure to establish a link between blood and tissue levels in both the general population and specific sub-populations, such as pregnant women. While extensive pharmacokinetic (PK) assessment is a pillar of the drug developmental process, it remains poorly characterized in the case of VD `├── LinkNode
│   full_match: `[14,15]`
│   inner_content: `14,15`
│   url: `14,15`
│   children:
│   ├── TextNode
│   │   full_match: `14,15`
│   │   inner_content: `14,15`├── TextNode
│   full_match: `. Therefore, the aim of this review is to elucidate the distribution of VD and its major metabolites in blood and tissue in response to oral VD supplementation. This knowledge is crucial in refining future trial designs and allows for more evidence-based evaluation of VD&#39;s efficacy in different body compartments.\n\n`
│   inner_content: `. Therefore, the aim of this review is to elucidate the distribution of VD and its major metabolites in blood and tissue in response to oral VD supplementation. This knowledge is crucial in refining future trial designs and allows for more evidence-based evaluation of VD&#39;s efficacy in different body compartments.\n\n`├── BoldNode
│   full_match: `__Section snippets__`
│   inner_content: `Section snippets`
│   children:
│   ├── TextNode
│   │   full_match: `Section snippets`
│   │   inner_content: `Section snippets`├── TextNode
│   full_match: `\nMetabolism of vitamin D\nSunlight exposure (UVB) initiates the synthesis of VD in the skin and evolutionarily was the primary source of VD. However, most of the world&#39;s populations spend 95 % of their time indoors, and even those who venture outside may use sunscreen, which could decrease exposure to UVB radiation. While dietary intake can modestly support VD reserves, few foods are fortified to a great extent. In contrast, structured oral supplementation leads to significantly increased VD levels (D2 and D3) compared …\n\n`
│   inner_content: `\nMetabolism of vitamin D\nSunlight exposure (UVB) initiates the synthesis of VD in the skin and evolutionarily was the primary source of VD. However, most of the world&#39;s populations spend 95 % of their time indoors, and even those who venture outside may use sunscreen, which could decrease exposure to UVB radiation. While dietary intake can modestly support VD reserves, few foods are fortified to a great extent. In contrast, structured oral supplementation leads to significantly increased VD levels (D2 and D3) compared …\n\n`├── BoldNode
│   full_match: `__1,25-Dihydroxyvitamin D&#39;s endocrine, paracrine, and autocrine effects__`
│   inner_content: `1,25-Dihydroxyvitamin D&#39;s endocrine, paracrine, and autocrine effects`
│   children:
│   ├── TextNode
│   │   full_match: `1,25-Dihydroxyvitamin D&#39;s endocrine, paracrine, and autocrine effects`
│   │   inner_content: `1,25-Dihydroxyvitamin D&#39;s endocrine, paracrine, and autocrine effects`├── TextNode
│   full_match: `\nThe endocrine action of the active metabolite, 1,25VD, has been well-characterized in contributing to the maintenance of serum calcium and phosphate homeostasis through the regulation of intestinal and renal (re)absorption. Recent research has shifted focus towards 1,25VD&#39;s autocrine and/or paracrine activities within tissues as potentially important for non-skeletal related effects. While the endocrine effects of 1,25VD are linked to the serum levels of 25VD, its autocrine and paracrine …\n\n`
│   inner_content: `\nThe endocrine action of the active metabolite, 1,25VD, has been well-characterized in contributing to the maintenance of serum calcium and phosphate homeostasis through the regulation of intestinal and renal (re)absorption. Recent research has shifted focus towards 1,25VD&#39;s autocrine and/or paracrine activities within tissues as potentially important for non-skeletal related effects. While the endocrine effects of 1,25VD are linked to the serum levels of 25VD, its autocrine and paracrine …\n\n`├── BoldNode
│   full_match: `__Vitamin D&#39;s biodistribution in blood__`
│   inner_content: `Vitamin D&#39;s biodistribution in blood`
│   children:
│   ├── TextNode
│   │   full_match: `Vitamin D&#39;s biodistribution in blood`
│   │   inner_content: `Vitamin D&#39;s biodistribution in blood`├── TextNode
│   full_match: `\nThe bioavailability of oral VD supplements exhibits significant inter-individual variability, with estimated rates between `
│   inner_content: `\nThe bioavailability of oral VD supplements exhibits significant inter-individual variability, with estimated rates between `├── BoldNode
│   full_match: `__55 and 99 %__`
│   inner_content: `55 and 99 %`
│   children:
│   ├── TextNode
│   │   full_match: `55 and 99 %`
│   │   inner_content: `55 and 99 %`├── TextNode
│   full_match: ` `
│   inner_content: ` `├── LinkNode
│   full_match: `[39]`
│   inner_content: `39`
│   url: `39`
│   children:
│   ├── TextNode
│   │   full_match: `39`
│   │   inner_content: `39`├── TextNode
│   full_match: `. Factors influencing VD absorption, and hence its bioavailability, include gastric and intestinal conditions (pH, gut enzymes and microbiome), other host-associated factors (baseline levels, age, morbidities and genetic variation `
│   inner_content: `. Factors influencing VD absorption, and hence its bioavailability, include gastric and intestinal conditions (pH, gut enzymes and microbiome), other host-associated factors (baseline levels, age, morbidities and genetic variation `├── LinkNode
│   full_match: `[40]`
│   inner_content: `40`
│   url: `40`
│   children:
│   ├── TextNode
│   │   full_match: `40`
│   │   inner_content: `40`├── TextNode
│   full_match: `), and the supplement formulation (e.g., tablets, solutions, gels) `
│   inner_content: `), and the supplement formulation (e.g., tablets, solutions, gels) `├── LinkNode
│   full_match: `[41,42]`
│   inner_content: `41,42`
│   url: `41,42`
│   children:
│   ├── TextNode
│   │   full_match: `41,42`
│   │   inner_content: `41,42`├── TextNode
│   full_match: `. Notably, VD absorption is not limited to passive …\n\n`
│   inner_content: `. Notably, VD absorption is not limited to passive …\n\n`├── BoldNode
│   full_match: `__Distribution of vitamin D and its metabolites across tissues following supplementation~~#00F: (radioactive tracking)~~__`
│   inner_content: `Distribution of vitamin D and its metabolites across tissues following supplementation~~#00F: (radioactive tracking)~~`
│   children:
│   ├── TextNode
│   │   full_match: `Distribution of vitamin D and its metabolites across tissues following supplementation`
│   │   inner_content: `Distribution of vitamin D and its metabolites across tissues following supplementation`
│   ├── ColorNode
│   │   full_match: `~~#00F: (radioactive tracking)~~`
│   │   inner_content: ` (radioactive tracking)`
│   │   attrs_dict:
│   │   │   raw_content: #00F
│   │   children:
│   │   ├── TextNode
│   │   │   full_match: `(radioactive tracking)`
│   │   │   inner_content: `(radioactive tracking)`├── TextNode
│   full_match: `\nThe 1972 study by Mawer et al. on the uptake of intravenously administered 14C-labelled VD in human tissues is foundational in elucidating the quantitative distribution and storage of VD following exogenous administration `
│   inner_content: `\nThe 1972 study by Mawer et al. on the uptake of intravenously administered 14C-labelled VD in human tissues is foundational in elucidating the quantitative distribution and storage of VD following exogenous administration `├── LinkNode
│   full_match: `[65]`
│   inner_content: `65`
│   url: `65`
│   children:
│   ├── TextNode
│   │   full_match: `65`
│   │   inner_content: `65`├── TextNode
│   full_match: `. Analyzing data from seven patients, Mawer et al. discovered a consistent pattern of rapid serum clearance of total radioactivity, followed by its redistribution to various tissues. `
│   inner_content: `. Analyzing data from seven patients, Mawer et al. discovered a consistent pattern of rapid serum clearance of total radioactivity, followed by its redistribution to various tissues. `├── BoldNode
│   full_match: `__Notably, the liver, kidneys, lungs, and spleen accumulated a minimal portion, &lt;2 %__`
│   inner_content: `Notably, the liver, kidneys, lungs, and spleen accumulated a minimal portion, &lt;2 %`
│   children:
│   ├── TextNode
│   │   full_match: `Notably, the liver, kidneys, lungs, and spleen accumulated a minimal portion, &lt;2 %`
│   │   inner_content: `Notably, the liver, kidneys, lungs, and spleen accumulated a minimal portion, &lt;2 %`├── TextNode
│   full_match: ` of the…\n\n`
│   inner_content: ` of the…\n\n`├── BoldNode
│   full_match: `__Vitamin D metabolite response during pregnancy~~#00F: (1,25 VD can double)~~__`
│   inner_content: `Vitamin D metabolite response during pregnancy~~#00F: (1,25 VD can double)~~`
│   children:
│   ├── TextNode
│   │   full_match: `Vitamin D metabolite response during pregnancy`
│   │   inner_content: `Vitamin D metabolite response during pregnancy`
│   ├── ColorNode
│   │   full_match: `~~#00F: (1,25 VD can double)~~`
│   │   inner_content: ` (1,25 VD can double)`
│   │   attrs_dict:
│   │   │   raw_content: #00F
│   │   children:
│   │   ├── TextNode
│   │   │   full_match: `(1,25 VD can double)`
│   │   │   inner_content: `(1,25 VD can double)`├── TextNode
│   full_match: `\nPregnancy presents a unique physiological state during which VD metabolism is significantly altered with a surge in its concentrations, indicating an extensive physiological requirement for this hormone. During pregnancy, the conversion rate of VD to 25VD remains comparable to those in the non-pregnant state `
│   inner_content: `\nPregnancy presents a unique physiological state during which VD metabolism is significantly altered with a surge in its concentrations, indicating an extensive physiological requirement for this hormone. During pregnancy, the conversion rate of VD to 25VD remains comparable to those in the non-pregnant state `├── LinkNode
│   full_match: `[120]`
│   inner_content: `120`
│   url: `120`
│   children:
│   ├── TextNode
│   │   full_match: `120`
│   │   inner_content: `120`├── TextNode
│   full_match: `. However, feedback regulation of 1,25VD becomes uncoupled, leading to a progressive increase in concentrations of 100–150 % upon conception and into the second trimester. This upward…\n\nDiscussion\nPrevious investigations have extensively characterized the changes to human serum 25VD after prolonged oral VD supplementation, identifying several individual-level modifiers of this response. Additionally, supplementation has been shown to increase serum 1,25VD in both pregnant and non-pregnant individuals, although significant increases in the latter are primarily observed during VD deficiency. Based on the summarized data from the reviewed studies (Table 1), it is evident that there&#39;s a…\n\nConclusions and future perspective\nThe current work provides insights into the distribution of VD and key metabolites throughout the body of various species. These findings have the potential to enhance research on the impact of VD supplementation, which may ultimately contribute to the development of more precise and effective supplementation guidelines as well as refined metrics for clinical studies. Future experimental research must rigorously investigate the serum and tissue levels of 1,25VD and 24,25VD in both humans and…`
│   inner_content: `. However, feedback regulation of 1,25VD becomes uncoupled, leading to a progressive increase in concentrations of 100–150 % upon conception and into the second trimester. This upward…\n\nDiscussion\nPrevious investigations have extensively characterized the changes to human serum 25VD after prolonged oral VD supplementation, identifying several individual-level modifiers of this response. Additionally, supplementation has been shown to increase serum 1,25VD in both pregnant and non-pregnant individuals, although significant increases in the latter are primarily observed during VD deficiency. Based on the summarized data from the reviewed studies (Table 1), it is evident that there&#39;s a…\n\nConclusions and future perspective\nThe current work provides insights into the distribution of VD and key metabolites throughout the body of various species. These findings have the potential to enhance research on the impact of VD supplementation, which may ultimately contribute to the development of more precise and effective supplementation guidelines as well as refined metrics for clinical studies. Future experimental research must rigorously investigate the serum and tissue levels of 1,25VD and 24,25VD in both humans and…`├── HorizontalRuleNode
│   full_match: `\n---\n`
│   inner_content: `---`├── HeadingNode
│   full_match: `!!!!VitaminDWiki – ((Calcidiol)) (25(OH) D) category contains:`
│   inner_content: `VitaminDWiki – ((Calcidiol)) (25(OH) D) category contains:`
│   level: `4`
│   children:
│   ├── TextNode
│   │   full_match: `VitaminDWiki – `
│   │   inner_content: `VitaminDWiki – `
│   ├── LocalLinkNode
│   │   full_match: `((Calcidiol))`
│   │   inner_content: `Calcidiol`
│   │   page: `Calcidiol`
│   │   children:
│   │   ├── TextNode
│   │   │   full_match: `Calcidiol`
│   │   │   inner_content: `Calcidiol`
│   ├── TextNode
│   │   full_match: ` (25(OH) D) category contains:`
│   │   inner_content: ` (25(OH) D) category contains:`├── TextNode
│   full_match: `\n`
│   inner_content: `\n`├── IncludeNode
│   full_match: `{include page=&quot;Calcidiol&quot; start=&quot;~tc~ start ~/tc~&quot; stop=&quot;~tc~ stop ~/tc~&quot;}`
│   inner_content: ``
│   attrs_dict:
│   │   raw_content:  page=&quot;Calcidiol&quot; start=&quot;~tc~ start ~/tc~&quot; stop=&quot;~tc~ stop ~/tc~&quot;
│   │   page: Calcidiol
│   │   start: ~tc~ start ~/tc~
│   │   stop: ~tc~ stop ~/tc~
│   children:
│   ├── TextNode
│   │   full_match: ``
│   │   inner_content: ``├── HorizontalRuleNode
│   full_match: `\n---\n`
│   inner_content: `---`├── HeadingNode
│   full_match: `!!!!((Calcitriol)) category  (1,25(OH)2D) contains;`
│   inner_content: `((Calcitriol)) category  (1,25(OH)2D) contains;`
│   level: `4`
│   children:
│   ├── LocalLinkNode
│   │   full_match: `((Calcitriol))`
│   │   inner_content: `Calcitriol`
│   │   page: `Calcitriol`
│   │   children:
│   │   ├── TextNode
│   │   │   full_match: `Calcitriol`
│   │   │   inner_content: `Calcitriol`
│   ├── TextNode
│   │   full_match: ` category  (1,25(OH)2D) contains;`
│   │   inner_content: ` category  (1,25(OH)2D) contains;`├── TextNode
│   full_match: `\n`
│   inner_content: `\n`├── IncludeNode
│   full_match: `{include page=&quot;Calcitriol&quot; start=&quot;~tc~ start ~/tc~&quot; stop=&quot;~tc~ end ~/tc~&quot;}`
│   inner_content: ``
│   attrs_dict:
│   │   raw_content:  page=&quot;Calcitriol&quot; start=&quot;~tc~ start ~/tc~&quot; stop=&quot;~tc~ end ~/tc~&quot;
│   │   page: Calcitriol
│   │   start: ~tc~ start ~/tc~
│   │   stop: ~tc~ end ~/tc~
│   children:
│   ├── TextNode
│   │   full_match: ``
│   │   inner_content: ``

Original Tiki:
-------------
{img type=&quot;attId&quot; attId=&quot;21562&quot; width=&quot;700&quot;}

{DIV(class=&quot;lefth4&quot;)}{maketoc Title=&quot;&quot;}{DIV}
---
!!!!Vitamin D beyond blood: Tissue distribution of vitamin D metabolites after supplementation
Life Sciences  10 August 2024 https://doi.org/10.1016/j.lfs.2024.122942 PDF behind paywall

__Highlights__
*Vitamin D3 and its key metabolites mediate a variety of important autocrine and paracrine mechanisms.
*These non-endocrine functions highlight the importance of vitamin D3 tissue stores.
*Data on human tissue distribution of vitamin D3 metabolites is lacking, particularly in the context of pregnancy.
*Within the examined dosage ranges, both human and animal studies indicate that vitamin D3 and its metabolites are retained in a dose-dependent manner that is tissue-specific.
*The blood-tissue kinetics and dynamics of vitamin D metabolites after supplementation should be further investigated to inform supplementation strategies.
Vitamin D3&#39;s role in mineral homeostasis through its endocrine function, associated with the main circulating metabolite 25-hydroxyvitamin D3, is well characterized. However, the increasing recognition of vitamin D3&#39;s paracrine and autocrine functions—such as cell growth, immune function, and hormone regulation—necessitates examining vitamin D3 levels across different tissues post-supplementation. Hence, this review explores the biodistribution of vitamin D3 in blood and key tissues following oral supplementation in humans and animal models, highlighting the biologically active metabolite, 1,25-dihydroxyvitamin D3, and the primary clearance metabolite, 24,25-dihydroxyvitamin D3. While our findings indicate significant progress in understanding how circulating metabolite levels respond to supplementation, comprehensive insight into their tissue concentrations remains limited. The gap is particularly significant during pregnancy, a period of drastically increased vitamin D3 needs and metabolic alterations, where data remains sparse. Within the examined dosage ranges, both human and animal studies indicate that vitamin D3 and its metabolites are retained in tissues selectively.
 Notably, vitamin D3 concentrations in tissues show greater variability in response to administered doses. In contrast, its metabolites maintain a more consistent concentration range, albeit different among tissues, reflecting their tighter regulatory mechanisms following supplementation. These observations suggest that serum 25-hydroxyvitamin D3 levels may not adequately reflect vitamin D3 and its metabolite concentrations in different tissues. Therefore, future research should aim to generate robust human data on the tissue distribution of vitamin D3 and its principal metabolites post-supplementation. Relating this data to clinically appropriate exposure metrics will enhance our understanding of vitamin D3&#39;s cellular effects and guide refinement of clinical trial methodologies.

__Introduction__
Vitamin D3 (VD), with its pleiotropic functional profile, plays an indispensable role in the regulation of bone mineral and immune homeostasis, and is deeply embedded in various physiological systems within the human body [1]. VD deficiency, a serum 25-hydroxyvitamin D (25VD) below 20 ng/ml as defined by the Institute of Medicine (IOM) based on skeletal health outcomes, is prevalent globally [2], [3], [4], [5]. The prevalence of low VD status may be significantly higher when considering the Endocrine Society&#39;s definition of deficiency, which is a serum 25VD level of &lt;30 ng/ml [6]. Despite the ubiquitous nature of VD deficiency, its health implications remain subject to debate. Numerous epidemiological studies have shown strong associations of VD deficiency with not only musculoskeletal health outcomes, but also a broad spectrum of adverse health outcomes, such as cancer, respiratory, neuro-developmental, auto-immune, cardiovascular and metabolic diseases [7]. On the other hand, randomized clinical trials (RCTs) examining the effect of oral VD supplementation on these health outcomes have generally been negative. This discrepancy has led some to question VD&#39;s role as a causal factor in non-skeletal health outcomes, implying association or even reverse-causality rather than a direct effect. However, the results of these RCT may have been inherently biased towards null findings. This bias can be attributed to the fact that the therapeutic-inspired research design and methodology of most trials failed to meet two critical requirements of nutritional intervention studies [8]. First, trials included large numbers of non-deficient subjects, diluting the potential to observe a true effect. Second, the one-size-fits-all (generic) approach to supplementation failed to ensure that all VD-deficient individuals achieved nutritional sufficiency [8]. When the intervention does not address a deficiency or fails to correct it adequately, evaluating the efficacy of the supplementation becomes impractical. This underscores a critical need for personalized supplementation regimens and reevaluation and novel insights into what constitutes VD sufficiency, extending beyond mere bone health [9].

In recent years, novel approaches have been explored to redefine VD sufficiency. For example, the free hormone hypothesis stipulates that it is primarily unbound VD metabolites that enter cells to exert their effect [10]. Certain physiological and clinical conditions might alter the balance between total and free 25VD, suggesting that measuring the free fraction might improve assessments of VD status [11]. Moreover, 25VD might be increasingly considered a great biomarker of exposure, but not necessarily of effect. Therefore, there has been a growing clinical interest in metabolites bearing closer proximity to VD&#39;s physiological actions, such as 1,25-dihydroxyvitamin D (1,25VD), the main biologically active metabolite, and 24,25(OH)2D (24,25VD), the main clearance metabolite. Finally, VD metabolite ratios (VMR) might be useful for accurately evaluating supplementation efficacy [12].

Refining supplementation efficacy necessitates not only a sense of the factors influencing supplementation response, such as body mass index (BMI) or interindividual variability in VD metabolism, but also a deep understanding of its distribution and subsequent target tissue concentrations required for an optimal physiological state [13]. As current guidelines are established based primarily on bone health, different health outcomes may benefit from varying thresholds [6]. More importantly, current research focusses almost exclusively on serum or plasma concentrations of VD and its metabolites, which does not fully capture the extent of tissue penetration post-supplementation. This oversight is significant because unlike drug trials that target specific serum concentrations to ensure immediate and systemic therapeutic efficacy, nutritional trials should aim for a more sustainable change in nutrient availability within the tissues and organs where its primary effects are exerted. Bridging this gap requires a profound understanding of how the body absorbs, distributes, and metabolizes the nutrient after circulatory exposure to establish a link between blood and tissue levels in both the general population and specific sub-populations, such as pregnant women. While extensive pharmacokinetic (PK) assessment is a pillar of the drug developmental process, it remains poorly characterized in the case of VD [14,15]. Therefore, the aim of this review is to elucidate the distribution of VD and its major metabolites in blood and tissue in response to oral VD supplementation. This knowledge is crucial in refining future trial designs and allows for more evidence-based evaluation of VD&#39;s efficacy in different body compartments.

__Section snippets__
Metabolism of vitamin D
Sunlight exposure (UVB) initiates the synthesis of VD in the skin and evolutionarily was the primary source of VD. However, most of the world&#39;s populations spend 95 % of their time indoors, and even those who venture outside may use sunscreen, which could decrease exposure to UVB radiation. While dietary intake can modestly support VD reserves, few foods are fortified to a great extent. In contrast, structured oral supplementation leads to significantly increased VD levels (D2 and D3) compared …

__1,25-Dihydroxyvitamin D&#39;s endocrine, paracrine, and autocrine effects__
The endocrine action of the active metabolite, 1,25VD, has been well-characterized in contributing to the maintenance of serum calcium and phosphate homeostasis through the regulation of intestinal and renal (re)absorption. Recent research has shifted focus towards 1,25VD&#39;s autocrine and/or paracrine activities within tissues as potentially important for non-skeletal related effects. While the endocrine effects of 1,25VD are linked to the serum levels of 25VD, its autocrine and paracrine …

__Vitamin D&#39;s biodistribution in blood__
The bioavailability of oral VD supplements exhibits significant inter-individual variability, with estimated rates between __55 and 99 %__ [39]. Factors influencing VD absorption, and hence its bioavailability, include gastric and intestinal conditions (pH, gut enzymes and microbiome), other host-associated factors (baseline levels, age, morbidities and genetic variation [40]), and the supplement formulation (e.g., tablets, solutions, gels) [41,42]. Notably, VD absorption is not limited to passive …

__Distribution of vitamin D and its metabolites across tissues following supplementation~~#00F: (radioactive tracking)~~__
The 1972 study by Mawer et al. on the uptake of intravenously administered 14C-labelled VD in human tissues is foundational in elucidating the quantitative distribution and storage of VD following exogenous administration [65]. Analyzing data from seven patients, Mawer et al. discovered a consistent pattern of rapid serum clearance of total radioactivity, followed by its redistribution to various tissues. __Notably, the liver, kidneys, lungs, and spleen accumulated a minimal portion, &lt;2 %__ of the…

__Vitamin D metabolite response during pregnancy~~#00F: (1,25 VD can double)~~__
Pregnancy presents a unique physiological state during which VD metabolism is significantly altered with a surge in its concentrations, indicating an extensive physiological requirement for this hormone. During pregnancy, the conversion rate of VD to 25VD remains comparable to those in the non-pregnant state [120]. However, feedback regulation of 1,25VD becomes uncoupled, leading to a progressive increase in concentrations of 100–150 % upon conception and into the second trimester. This upward…

Discussion
Previous investigations have extensively characterized the changes to human serum 25VD after prolonged oral VD supplementation, identifying several individual-level modifiers of this response. Additionally, supplementation has been shown to increase serum 1,25VD in both pregnant and non-pregnant individuals, although significant increases in the latter are primarily observed during VD deficiency. Based on the summarized data from the reviewed studies (Table 1), it is evident that there&#39;s a…

Conclusions and future perspective
The current work provides insights into the distribution of VD and key metabolites throughout the body of various species. These findings have the potential to enhance research on the impact of VD supplementation, which may ultimately contribute to the development of more precise and effective supplementation guidelines as well as refined metrics for clinical studies. Future experimental research must rigorously investigate the serum and tissue levels of 1,25VD and 24,25VD in both humans and…
---
!!!!VitaminDWiki – ((Calcidiol)) (25(OH) D) category contains:
{include page=&quot;Calcidiol&quot; start=&quot;~tc~ start ~/tc~&quot; stop=&quot;~tc~ stop ~/tc~&quot;}
---
!!!!((Calcitriol)) category  (1,25(OH)2D) contains;
{include page=&quot;Calcitriol&quot; start=&quot;~tc~ start ~/tc~&quot; stop=&quot;~tc~ end ~/tc~&quot;}
</code>
</pre>
